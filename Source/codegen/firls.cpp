...
void firls(const double varargin_3[64], double h_data[], int h_size[2])
{
  ...
  double b_x_data[65];
  static double b_tmp_data[4225];
  double c_tmp_data[4225];
  double d_tmp_data[4225];
  double e_tmp_data[4225];
  for (i = 0; i < 65; i++) {
  ...
  for (i = 0; i < 64; i++) {
    tmp_data[i] = 6.2831853071795862 * static_cast<double>(k_data[i]);
  }
  ...
  for (I1_data_tmp = 0; I1_data_tmp < 32; I1_data_tmp++) {
    ...
    for (i = 0; i < 64; i++) {
      b_data[i] += a * (x[i] - b_x[i]) / static_cast<double>(k_data[i] *
        k_data[i]);
    }
    ...
    for (i = 0; i < 4225; i++) {
      b_tmp_data[i] = 2.0 * static_cast<double>(I1_data[i]) * m_s_tmp_tmp;
    }
    ...
    for (i = 0; i < 4225; i++) {
      c_tmp_data[i] = 2.0 * static_cast<double>(I2_data[i]) * m_s_tmp_tmp;
    }
    ...
    for (i = 0; i < 4225; i++) {
      d_tmp_data[i] = 2.0 * static_cast<double>(I1_data[i]) * m_s_tmp;
    }
    ...
    for (i = 0; i < 4225; i++) {
      e_tmp_data[i] = 2.0 * static_cast<double>(I2_data[i]) * m_s_tmp;
    }

    a = 0.5 * m_s_tmp_tmp;
    b_m_s_tmp = 0.5 * m_s_tmp;
    for (s = 0; s < 4225; s++) {
      if (std::abs(e_tmp_data[s]) < 1.0020841800044864E-292) {
        m_s = 1.0;
        e_tmp_data[s] = 1.0;
      } else {
        m_s = 3.1415926535897931 * e_tmp_data[s];
        m_s = std::sin(m_s) / m_s;
        e_tmp_data[s] = m_s;
      }

      G_data[s] += a * (b_tmp_data[s] + c_tmp_data[s]) - b_m_s_tmp *
        (d_tmp_data[s] + m_s);
    }
  }
